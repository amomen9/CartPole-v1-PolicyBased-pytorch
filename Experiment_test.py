...
def experiment():
    ...
    global_config = {
        ...
        # Agent
        "n_timesteps": 15e3, #1e6,              # Total number of training timesteps. Default: 1000000.
    ...
    PPO_config = {
        ...
        "n_epochs": [40],                 # 15   # of optimisation epochs per rollout which controls how many times we reuse each collected rollout batch of data to update the policy. Default: 10. Set to 1 to skip PPO epoch trials and only do one epoch per rollout.
    ...
    # If any algorithm flag is passed, it overrides `included_algo_learnings` above.
    ...
    if any(cli_selected.values()):
        included_algo_learnings = cli_selected
