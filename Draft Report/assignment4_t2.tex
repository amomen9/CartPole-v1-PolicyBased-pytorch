...
\end{enumerate}
\textbf{Proximal Policy Optimisation} (PPO) addresses both issues by reusing each batch of on-policy data for several gradient epochs (running off-policy for batches) and by clipping the importance ratio so that the new policy never strays too far from the policy that collected the data.
In addition to the base PPO algorithm, we also have used Generalised Advantage Estimation (GAE) \cite{schulman2016gae} extension for the advantage estimator. There are other extensions such as adding entropy but are not studied in this article. \autoref{sec:theory} introduces the relevant equations, derives the clipped objective from the policy-gradient objective used in A2C, and presents the algorithm pseudocode. \autoref{sec:experiments} describes the environment and experimental design. The learning curves are reported in \autoref{sec:results} and analysed in \autoref{sec:discussion}. We conclude in \autoref{sec:conclusion}.

\section{Theory}\label{sec:theory}

\subsection{From policy gradient to PPO}\label{sec:theory:pg}

A stochastic policy $\pi_\theta(a|s)$ assigns a probability to each action $a$ in state $s$ given parameters $\theta\in\mathbb{R}^{d}$. The objective is the expected discounted return $J(\theta)=\mathbb{E}_{\tau\sim\pi_\theta}\!\left[\sum_{t=0}^{H-1}\gamma^t r_t\right]$ where $\gamma\in[0,1]$ is the discount factor and $\tau=\{s_0,a_0,r_0,\dots,s_H\}$ is a trajectory of horizon $H$ sampled from $\pi_\theta$. The policy-gradient theorem expresses its gradient as
\begin{equation}
    \nabla_\theta J(\theta) = \mathbb{E}_{\tau\sim\pi_\theta}\!\left[\sum_{t=0}^{H-1} \hat A_t \nabla_\theta \log\pi_\theta(a_t|s_t)\right],
    \label{eq:pg}
\end{equation}
...
We use the CartPole-v1 environment from Gymnasium. The state $s\in\mathbb{R}^{4}$ is $(\text{cart position},\text{cart velocity},\text{pole angle},\text{pole angular velocity})$. The discrete action space is $\{\text{left},\text{right}\}$. A reward of $+1$ is given for every timestep the pole remains within $12^{\circ}$ of vertical and the cart within $(-2.4,2.4)$. Episodes are truncated at $500$ steps, so the per-episode return is bounded above by $500$.

\subsection{Implementation and engineering tricks}\label{sec:experiments:impl}

We implement PPO-clipped from scratch in PyTorch following \autoref{alg:PPO}. The actor and critic are independent fully-connected networks with $\tanh$ activations. The implementation contains exactly one ingredient beyond the basic policy-gradient method of A2C, and it is part of the algorithm as published:
...
Each of the $K$ inner epochs is a single full-batch gradient step on the entire rollout; no mini-batching is used. We also deliberately omit value-function clipping, entropy bonuses, advantage normalisation and global gradient-norm clipping. These additions are common in PPO implementations \cite{engstrom2020implementation} but are not part of the core algorithm; reporting numbers without them isolates the contribution of \cref{eq:clip} and \cref{eq:gae}.

\subsection{Experimental setup}\label{sec:experiments:setup}

Each configuration is trained for one million environment steps. Returns are recorded every $250$ steps. Each learning curve is averaged over five independent seeds; we report the mean and a Student-$t$ confidence band around it. Curves shown in the main text are smoothed with a rolling window of width $201$; raw curves are provided in the appendix. As in Assignments 2 and 3, learning rate and network width were tuned first; the other hyperparameters were then varied around reasonable defaults from. The setting used in \autoref{fig:main_result} is $\alpha=3\!\times\!10^{-4}$, $\beta=10^{-3}$, $\gamma=0.99$, $\lambda=0.95$, $\epsilon=0.2$, rollout $T=2048$, $K=10$ epochs, two hidden layers of $64$ units in both networks. We choose $T$ as a power of two by convention, inherited from the original PPO implementation: the algorithm does not require it, but powers of two align rollout and mini-batch tensors with the multiples of $32$ used by GPU tensor cores, which is the standard deep-learning batching convention. The exact configuration file is included with the source code.
